...
(function($){
    ...
    OnelineliteAdmin = {
        ...
        _homePageSetup: function() {
            ...
            OnelineliteAdmin._loaderActive($class,"Home Page Setup");
            var data = {
                'action': 'default_home',
                'home': 'saved'
            };
        ...
        _activetedPlugin: function(event, args){
                event.preventDefault();
                var $card = jQuery( '.'+args.slug);
                OnelineliteAdmin._activePluginHomepage(args.slug,$card.data('init'));
        },
        /**
        ...
        _activePlugin: function(event){
                ...
                OnelineliteAdmin._activePluginHomepage($slug,$init);
            },
        _bind: function(){               
            $( document ).on('click'                     , '.default-home', OnelineliteAdmin._homePageSetup);
            $( document ).on('click'                     , '.install-now', OnelineliteAdmin._installNow);
            $( document ).on('click'                     , '.activate-now', OnelineliteAdmin._activePlugin);
            $( document ).on('wp-plugin-install-error'   , OnelineliteAdmin._installError);
            $( document ).on('wp-plugin-installing'      , OnelineliteAdmin._pluginInstalling);
            $( document ).on('wp-plugin-install-success' , OnelineliteAdmin._activetedPlugin);  
        },


};
OnelineliteAdmin.init();
/**
         * TABS
         */
