...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a target="_blank" class="plugin-detail thickbox1 open-plugin-details-modal1" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'View Details', 'oneline-lite' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            if ( ! is_plugin_active( $plugin_init ) ){
                ...
                $detail_link = add_query_arg(
                    ...
                    $pluginArr['detail_link']= 'https://themehunk.com/plugins/'; //$detail_link;
